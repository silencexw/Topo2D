...
class MapTRPerceptionTransformer(BaseModule):
    ...
    def forward(self,
        ...
        # import pdb; pdb.set_trace()
        bs = mlvl_feats[0].size(0)
        # bev_embed = nn.Parameter(torch.Tensor(
        #     1,20000, 256)).to(target.device)

        # target[torch.isnan(target)] = 0.5
        # reference_points[torch.isnan(reference_points)] = 0.1
        # query_embeds[torch.isnan(query_embeds)] = 0.2
        init_reference_out = reference_points
        # breakpoint()
        inter_states, inter_references = self.decoder(
            query=target.permute(1, 0, 2),
            key=None,
            value=bev_embed.permute(1, 0, 2),
            query_pos=query_embeds.permute(1, 0, 2),
            reference_points=reference_points,
            reg_branches=reg_branches,
            cls_branches=cls_branches,
            spatial_shapes=torch.tensor([[bev_h, bev_w]], device=target.device),
            level_start_index=torch.tensor([0], device=target.device),
